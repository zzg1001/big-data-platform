...
"""
    ...
    def chat_for_dimension_tagging(
        ...
        system_prompt = f"""你是标签助手。当前是【{dimension_name}】实体，一次性列出所有可做的基础标签供用户勾选。

**重要：用户在对话开头说的话决定了宽表的命名！**
- 用户说"帮我做行为标签" → 宽表名为"行为数据"
- 用户说"帮我做基础标签" → 宽表名为"基础信息"
- 请仔细阅读对话历史，找到用户的原始请求来确定宽表主题
...
根据用户在对话中说的主题来命名宽表！返回JSON对象：
{{
  "type": "dimension_result",
  "wide_table_display": "（根据用户请求推断，如：行为数据、基础信息、消费画像）",
  "wide_table_name": "tag_{{entity}}_{{theme}}_{{YYYYMMDD}}",
  "dimension_tags": [
    {{"type": "dimension_tags", "type_name": "xxx标签", "type_description": "xxx", "tags": [...], "sql": "SELECT ..."}},
    ...
  ]
}}

## 宽表命名规则（最重要！必须根据用户的原始请求来命名）

**核心原则：根据用户在对话开头说的话来命名宽表！**

用户说"帮我生成用户的行为标签" → wide_table_display="行为数据", wide_table_name="tag_user_behavior_YYYYMMDD"
用户说"帮我做基础信息标签" → wide_table_display="基础信息", wide_table_name="tag_user_basic_info_YYYYMMDD"
用户说"帮我做消费画像标签" → wide_table_display="消费画像", wide_table_name="tag_user_consumption_YYYYMMDD"
用户说"帮我做用户偏好标签" → wide_table_display="用户偏好", wide_table_name="tag_user_preference_YYYYMMDD"

**命名格式：**
- wide_table_display: 中文展示名，从用户请求中提取主题词，如"行为数据"、"基础信息"、"消费画像"
- wide_table_name: tag_{{entity}}_{{theme}}_{{YYYYMMDD}}
  - entity: 从id_field推断，如 user、order、product
  - theme: 主题英文，如 behavior、basic_info、consumption、preference
  - YYYYMMDD: 当前日期

**绝对禁止：**
- 不要把标签名拼接成展示名！如"出差频次_消费水平_酒店评价"是错误的
- 不要用时间戳代替主题！如"tag_user_1234567890"是错误的

## 规则
- 首次：返回 tag_suggestions
- 确认时：返回 dimension_result 对象（包含宽表命名 + dimension_tags数组）
- 每个dimension_tags对象必须包含: type, type_name, type_description, tags, sql
        ...
        if '"type":' in response or '"type" :' in response:
            try:
                ...
                if json_start >= 0 and json_end > json_start:
                    ...
                    # 新格式：dimension_result 包含宽表信息和标签数组
                    if isinstance(parsed, dict) and parsed.get("type") == "dimension_result":
                        dimension_tags = parsed.get("dimension_tags", [])
                        if dimension_tags:
                            result["dimension_tags_list"] = dimension_tags
                            result["wide_table_display"] = parsed.get("wide_table_display", "")
                            result["wide_table_name"] = parsed.get("wide_table_name", "")
                            result["is_final"] = True
                            result["reply"] = f"已生成 {len(dimension_tags)} 个类型标签"
                            logger.info("[AI Dimension Chat] AI返回了%d个类型标签，宽表: %s",
                                        len(dimension_tags), parsed.get("wide_table_display"))
                    # 兼容旧格式：数组形式
...
"""
    ...
    def infer_entity_id_field(self, entity_name: str) -> dict:
        """
        根据实体名称推断 ID 字段（英文）
        ...
        返回: {
            "name": "user",           # 英文标识
            "id_field": "user_id",    # ID字段名
            "success": True
        }
        """
        system_prompt = """你是数据架构师。根据实体名称推断对应的英文标识和ID字段名。

规则：
1. 将中文实体名翻译成简洁的英文单词（小写）
2. ID字段格式：英文名_id
3. 常见映射：用户->user, 商品->product, 订单->order, 客户->customer, 会员->member, 员工->employee, 部门->department, 设备->device, 店铺->shop/store, 交易->transaction

返回JSON格式（只返回JSON，不要其他内容）：
{"name": "英文标识", "id_field": "xxx_id"}

示例：
输入：用户
输出：{"name": "user", "id_field": "user_id"}

输入：商品实体
输出：{"name": "product", "id_field": "product_id"}
"""

        user_prompt = f"实体名称：{entity_name}"

        logger.info("[AI Infer Entity] 实体名称: %s", entity_name)

        try:
            response = self._call_claude(system_prompt, user_prompt)
            logger.info("[AI Infer Entity] AI 响应: %s", response)
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                return {
                    ...
                    "id_field": data.get("id_field", "entity_id"),
                    "success": True
                }
        except Exception as e:
            logger.warning("[AI Infer Entity] 推断失败: %s", str(e))
...
注意：理解用户的真实意图，灵活处理各种表达方式。"""
    ...
    def infer_table_name(self, display_name: str, entity_name: str) -> dict:
        """
        根据中文展示名推断英文表名
        ...
        system_prompt = """你是一个翻译助手。将中文主题词翻译成简短的英文标识符。

规则：
1. 返回一个简短的英文单词或下划线连接的词组
2. 使用小写字母和下划线
3. 常见翻译：
   - 基础信息/基础数据/基本信息 → basic_info
   - 行为数据/行为特征/行为标签 → behavior
   - 消费画像/消费数据/消费特征 → consumption
   - 用户偏好/偏好数据 → preference
   - 活跃度/活跃数据 → activity
   - 价值分层/用户价值 → value
   - 生命周期 → lifecycle
   - 会员等级 → membership
   - 地域分布/地理位置 → region
4. 如果不在上面列表中，根据语义推断一个合适的英文词

只返回英文单词，不要其他内容。"""

        user_prompt = f"翻译：{display_name}"

        try:
            theme = self._call_claude(system_prompt, user_prompt, temperature=0.1).strip().lower()
            # 清理可能的多余字符
            theme = re.sub(r'[^a-z0-9_]', '', theme)
            if not theme:
                theme = "data"
        except Exception as e:
            logger.warning(f"AI推断表名失败: {e}")
            theme = "data"

        table_name = f"tag_{entity_name}_{theme}_{date_str}"
